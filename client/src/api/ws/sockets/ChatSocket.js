  ...
  onNewMessage = () => {
    this.socket.on('newMessage', (data) => {
      ...
      messagesPreview.forEach((msgPreview) => {
        if (isEqual(msgPreview.participants, message.participants)) {
          msgPreview.text = message.body;
          msgPreview.sender = message.sender;
          msgPreview.createAt = message.createdAt;
          isNew = false;
        }
      });
      if (isNew) {
